...
class Result {
	...
	function f_top10_toys() {
		$query = "select toy,count(*) as cnt from prefs where toy <> '' AND device_id not in (select device_id from banned) AND LOWER(toy) not regexp (select group_concat(word SEPARATOR '|') from banned_words) group by toy order by cnt DESC LIMIT 10 ;" ;
                $result = $this->db->query($query) or die(mysql_error());
		unset($top10_toys) ;
		$top10_toys = array() ;
	...
	function f_top10_toys_time($time,$comment,$substring) {
		if ( $time == "" ) { $time = "1 YEAR" ; } 
		$query = "select toy,count(*) as cnt from prefs where toy <> '' AND update_time > DATE_SUB(NOW(), INTERVAL $time) AND device_id not in (select device_id from banned) AND LOWER(toy) not regexp (select group_concat(word SEPARATOR '|') from banned_words) group by toy order by cnt DESC LIMIT 10 ;" ;
                $result = $this->db->query($query) or die(mysql_error());
		unset($top10_toys) ;
	...
	function f_top10_kids_name_status($status) {
		...
		$query = "select SUBSTRING_INDEX(name,' ',1), count(*) as cnt from prefs where name <> '' AND status='$status' AND device_id not in (select device_id from banned) AND LOWER(name) not regexp (select group_concat(word SEPARATOR '|') from banned_words) group by name order by cnt desc limit 10;" ;
                $result = $this->db->query($query) or die(mysql_error());
	...
	function f_top10_toys_year($year) {
		$query = "select toy,count(*) as cnt from prefs where toy <> '' AND YEAR (update_time) = '$year' AND device_id not in (select device_id from banned) AND LOWER(toy) not regexp (select group_concat(word SEPARATOR '|') from banned_words) group by toy order by cnt DESC LIMIT 10 ;" ;
                $result = $this->db->query($query) or die(mysql_error());
	...
	function f_top10_toys_year_country($year,$country) {
		$query = "select prefs.toy,count(*) as cnt from prefs,phone  where toy <> '' AND phone.country='$country' AND prefs.device_id = phone.device_id AND prefs.device_id not in (select device_id from banned) AND LOWER(toy) not regexp (select group_concat(word SEPARATOR '|') from banned_words) group by toy order by cnt desc limit 10 ;" ;
                $result = $this->db->query($query) or die(mysql_error());
	...
	function f_top10_toys_status($status) {
		...
		$query = "select toy,count(*) as cnt from prefs where toy <> '' AND status = 'g' AND device_id not in (select device_id from banned) AND LOWER(toy) not regexp (select group_concat(word SEPARATOR '|') from banned_words) group by toy order by cnt DESC LIMIT 10 ;" ;
                $result = $this->db->query($query) or die(mysql_error());
	...
	function f_top10_toys_age($from, $to) {
		$query = "select toy,count(*) as cnt from prefs where toy <> '' AND age >= $from and age <= $to AND device_id not in (select device_id from banned) AND LOWER(toy) not regexp (select group_concat(word SEPARATOR '|') from banned_words) group by toy order by cnt DESC LIMIT 10 ;" ;
                $result = $this->db->query($query) or die(mysql_error());
